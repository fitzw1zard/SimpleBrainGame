...
import com.example.simplebraingame.data.GameRepositoryImpl
...
import com.example.simplebraingame.domain.usecases.GenerateQuestionUseCase
import com.example.simplebraingame.domain.usecases.GetGameSettingsUseCase
...
class GameViewModel(
    ...
    private fun updateProgress() {
        ...
        _enoughPercentRightAnswers.value = percent >= gameSettings.minPercentOfRightAnswers
    }

    private fun calculatePercentOfRightAnswers(): Int {
        if (countOfQuestions == 0) return 0
        return (countOfRightAnswers * 100 / countOfQuestions.toDouble()).toInt()
    }
    ...
    private fun startTimer() {
        timer = object : CountDownTimer(
            gameSettings.gameTime * MSEC_IN_SEC,
            MSEC_IN_SEC
        ) {
    ...
    private fun formatTime(millisUntilFinished: Long): String {
        val seconds = millisUntilFinished / MSEC_IN_SEC
    ...
    private fun finishGame() {
        _gameResult.value = GameResult(
            enoughPercentRightAnswers.value == true && enoughRightAnswers.value == true,
            countOfRightAnswers,
            countOfQuestions,
    ...
    companion object {
        private const val MSEC_IN_SEC = 1000L
